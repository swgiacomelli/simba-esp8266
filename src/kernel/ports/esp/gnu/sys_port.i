...
/* The main stack is placed before all other stacks in the memory. */
uint32_t main_stack[DIV_CEIL(
    sizeof(struct thrd_t) + (CONFIG_SYS_SIMBA_MAIN_STACK_MAX),
    sizeof(uint32_t))] __attribute__((section(".simba_main_stack")));
...
static int sys_port_module_init(void) {
  ...
  ESP8266_TIMER0->CTRL =
      (ESP8266_TIMER_CTRL_ENABLE | ESP8266_TIMER_CTRL_AUTO_LOAD |
       SYS_TICK_TIMER_PRESCALE | ESP8266_TIMER_CTRL_INT_TRIGGER_EDGE);
...
static int sys_port_backtrace(void **buf_pp, size_t size) { return (0); }

static int sys_port_get_time_into_tick() { return (0); }

static void sys_port_lock(void) { portDISABLE_INTERRUPTS(); }

static void sys_port_unlock(void) { portENABLE_INTERRUPTS(); }

static void sys_port_lock_isr(void) {}

static void sys_port_unlock_isr(void) {}

static cpu_usage_t sys_port_interrupt_cpu_usage_get(void) { return (0); }
...
void user_init() {
  ...
  xTaskGenericCreate(main_task, (const signed char *)"simba",
                     ((size_t)&__simba_stack_size) / sizeof(int), NULL, 4, NULL,
                     (long unsigned int *)(&__simba_stack_begin), NULL);
}
