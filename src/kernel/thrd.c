...
static size_t stack_heap_fixed_buffer_sizes[HEAP_FIXED_SIZES_MAX] = {
    0,
};
#endif
...
static int set_env(struct thrd_environment_t *env_p, const char *name_p,
                   const char *value_p) {
  int i;
...
static int thrd_get_used_stack(struct thrd_t *thrd_p) {
  ...
  /* Stack grows towards lower memory addresses, so start from the
     bottom.*/
  while ((i < thrd_p->stack_size) && (stack_p[i] == THRD_FILL_PATTERN)) {
    i++;
  }
...
static char *const FAR state_fmt[] = {"current", "ready", "suspended",
                                      "resumed", "terminated"};

static int cmd_list_cb(int argc, const char *argv[], void *chout_p,
                       void *chin_p, void *arg_p, void *call_arg_p) {
  struct thrd_t *thrd_p;

  std_fprintf(chout_p, OSTR("                NAME        STATE  PRIO"
...
#endif
                     "     0x%02x\r\n"),
                thrd_p->name_p, state_fmt[thrd_p->state], thrd_p->prio,
#if CONFIG_THRD_CPU_USAGE == 1
#if CONFIG_FLOAT == 1
...
#endif
#if CONFIG_PROFILE_STACK == 1
                thrd_get_used_stack(thrd_p), (int)thrd_p->stack_size,
#endif
                thrd_p->log_mask);
...
static int cmd_set_log_mask_cb(int argc, const char *argv[], void *chout_p,
                               void *chin_p, void *arg_p, void *call_arg_p) {
  struct thrd_t *thrd_p;
  long mask;
...
int thrd_module_init(void) {
...
#if CONFIG_THRD_STACK_HEAP == 1
  heap_init(&stack_heap, &stack_heap_buffer[0], sizeof(stack_heap_buffer),
  ...
  thrd_p->next_p = NULL;
  thrd_p->stack_size =
      (thrd_port_get_main_thrd_stack_top() - (char *)(thrd_p + 1));
...
#if CONFIG_MONITOR_THREAD == 1
  thrd_spawn(monitor_main, NULL, THRD_MONITOR_PRIO, monitor_thrd_stack,
             sizeof(monitor_thrd_stack));
#endif

#if CONFIG_THRD_FS_COMMANDS == 1
  fs_command_init(&module.cmd_list, CSTR("/kernel/thrd/list"), cmd_list_cb,
                  NULL);
  fs_command_register(&module.cmd_list);

  fs_command_init(&module.cmd_set_log_mask, CSTR("/kernel/thrd/set_log_mask"),
                  cmd_set_log_mask_cb, NULL);
  ...
  fs_command_init(&module.cmd_monitor_set_period_ms,
                  ...
                  cmd_monitor_set_period_ms_cb, NULL);
...
int thrd_terminate(struct thrd_t *thrd_p) {
  sys_lock();
  thrd_prio_list_remove_isr(&module.scheduler.ready, &thrd_p->scheduler.elem);
#if CONFIG_THRD_TERMINATE == 1
  sem_give_isr(&thrd_self()->join_sem, 1);
#endif
...
int thrd_sleep(float seconds) { return (thrd_sleep_us(1000000 * seconds)); }
...
const char *thrd_get_name() { return (thrd_self()->name_p); }
...
int thrd_get_log_mask(void) { return (module.scheduler.current_p->log_mask); }
...
int thrd_get_prio(void) { return (module.scheduler.current_p->prio); }
  ...
  if (thrd_p->state == THRD_STATE_RESUMED) {
    ...
    if (timeout_p != NULL) {
      if ((timeout_p->seconds <= 0) && (timeout_p->nanoseconds <= 0)) {
        ...
        timer_init(&timer, timeout_p, on_suspend_timer_expired, thrd_p, 0);
